const BUILD_VERSION = '3.0.0-alpha255';
...
const STACKUP_PDFJS_URL = './pdf.mjs?v=3.0.0-alpha255';
const STACKUP_PDFJS_WORKER_URL = './pdf.worker.mjs?v=3.0.0-alpha255';
...
navigator.serviceWorker.register('service-worker.js?v=3.0.0-alpha255', { updateViaCache: 'none' }).then((registration) => registration.update()).catch(() => {});
  });
}
...
window.addEventListener('load', async () => {
  ...
  reportJobsSyncStatus(`Sync ready (3.0.0-alpha255). ${waiting ? waiting + ' local job(s) waiting. Tap Sync to upload.' : 'Local jobs are up to date.'}`);
...
/* ===== 3.0.0-alpha255 auto-scroll guard ===== */
(function(){
...
(function(){
const TC63_VERSION = '3.0.0-alpha255';
...
/* ===== 3.0.0-alpha255 shared library row consistency ===== */
(function(){
  const $ = (id) => document.getElementById(id);
