/* TapCalc Dev 3.0.0-alpha212 inline field manual reference */
(function(){
  const STYLE_ID = 'tapcalc-alpha212-fieldmanual-style';
  const READY_FLAG = '__tapcalcAlpha212FieldManualReady';
  ...
  function showFieldManual(){
    ...
    try { localStorage.setItem('tapcalcAlpha212FieldManualOpen', '1'); } catch {}
  ...
  function hideFieldManual(){
    ...
    try { localStorage.removeItem('tapcalcAlpha212FieldManualOpen'); } catch {}
  }
  ...
  function installFieldManualReference(){
    ...
    document.body.classList.add('tapcalc-alpha212-fieldmanual');
    ...
    setTimeout(() => {
      ensureFieldManualTriggers();
      try {
        if (localStorage.getItem('tapcalcAlpha212FieldManualOpen') === '1') showFieldManual();
      } catch {}
    }, 450);
  }
