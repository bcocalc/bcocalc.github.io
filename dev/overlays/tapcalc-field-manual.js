/* TapCalc Dev 3.0.0-alpha214 inline field manual reference */
(function(){
  const STYLE_ID = 'tapcalc-alpha214-fieldmanual-style';
  const READY_FLAG = '__tapcalcAlpha214FieldManualReady';
  ...
  let activeFieldManualSection = 'Thread Make-up';
  function injectFieldManualStyles(){
    ...
    style.textContent = `
      ...
      body.measurement-page #refScreen .field-manual-topic-grid{display:none;}
      ...
      body.measurement-page #refScreen .field-manual-mobile-nav{display:grid;gap:7px;margin:12px 0 2px;padding:10px;border-radius:16px;border:1px solid rgba(132,181,235,.32);background:rgba(5,19,38,.82);}
      body.measurement-page #refScreen .field-manual-mobile-nav label{font-size:.72rem;letter-spacing:.12em;text-transform:uppercase;color:rgba(224,236,255,.74);font-weight:850;}
      body.measurement-page #refScreen .field-manual-mobile-nav select{width:100%;border-radius:12px;border:1px solid rgba(142,190,244,.42);background:#07182f;color:#fff;padding:11px 12px;font-weight:850;}
      body.measurement-page #refScreen .field-manual-mobile-nav p{margin:0;color:rgba(225,237,255,.74);font-size:.82rem;}
      body.measurement-page #refScreen .field-manual-section{margin-top:16px;}
      body.measurement-page #refScreen .tapcalc-fieldmanual-view:not(.field-manual-searching) .field-manual-section:not(.field-manual-section-active){display:none;}
      body.measurement-page #refScreen .tapcalc-fieldmanual-view:not(.field-manual-searching) .field-manual-section.field-manual-section-active{display:block;}
      ...
      @media (max-width:760px){body.measurement-page #refScreen .field-manual-entry-row{grid-template-columns:1fr;}body.measurement-page #refScreen .field-manual-hero{padding:14px;border-radius:18px;}body.measurement-page #refScreen .field-manual-card-grid{grid-template-columns:1fr;}body.measurement-page #refScreen .field-manual-table{min-width:420px;}}
      @media (max-width:430px){body.measurement-page #refScreen .field-manual-topic-grid{grid-template-columns:1fr;}body.measurement-page #refScreen .field-manual-table{min-width:360px;}}
      :root[data-theme="light"] body.measurement-page #refScreen .field-manual-mobile-nav,body.light.measurement-page #refScreen .field-manual-mobile-nav{background:rgba(255,255,255,.76);border-color:rgba(76,123,174,.26);} 
      :root[data-theme="light"] body.measurement-page #refScreen .field-manual-mobile-nav label,:root[data-theme="light"] body.measurement-page #refScreen .field-manual-mobile-nav p,body.light.measurement-page #refScreen .field-manual-mobile-nav label,body.light.measurement-page #refScreen .field-manual-mobile-nav p{color:#315171;}
      :root[data-theme="light"] body.measurement-page #refScreen .field-manual-mobile-nav select,body.light.measurement-page #refScreen .field-manual-mobile-nav select{background:#f8fbff;color:#071d35;border-color:rgba(74,118,169,.36);}
    `;
  ...
  function renderFieldManualView(workspace){
    ...
    view.innerHTML = `
      <div class="field-manual-hero" data-field-manual-text="field manual reference thread make-up cutter rpm hi-stop machine specs post-job checks tolerance">
        ...
        <div class="field-manual-mobile-nav">
          <label for="fieldManualSectionSelect">Field Manual section</label>
          <select id="fieldManualSectionSelect">
            ${topicCards.map((card) => `<option value="${escapeHtml(card.title)}">${escapeHtml(card.title)}</option>`).join('')}
          </select>
          <p>Pick one section at a time. Search opens every matching section.</p>
        </div>
    ...
    `;
    applyFieldManualCompactState();
  }

  function applyFieldManualCompactState(options = {}){
    const view = document.getElementById('fieldManualReferenceView');
    if (!view) return;
    const query = document.getElementById('fieldManualSearch')?.value?.trim() || '';
    view.classList.add('field-manual-compact-mode');
    view.classList.toggle('field-manual-searching', Boolean(query));
    const select = document.getElementById('fieldManualSectionSelect');
    if (select && select.value !== activeFieldManualSection) select.value = activeFieldManualSection;
    document.querySelectorAll('#fieldManualReferenceView .field-manual-section').forEach((section) => {
      const isActive = section.getAttribute('data-field-manual-section') === activeFieldManualSection;
      section.classList.toggle('field-manual-section-active', Boolean(query) || isActive);
    });
    if (options.scroll && !query) {
      const target = document.querySelector(`#fieldManualReferenceView [data-field-manual-section="${options.cssEscape ? options.cssEscape(activeFieldManualSection) : activeFieldManualSection.replace(/["\\]/g, '\\$&')}"]`);
      setTimeout(() => target?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 0);
    }
  }

  function setActiveFieldManualSection(sectionName, options = {}){
    if (topicCards.some((card) => card.title === sectionName)) activeFieldManualSection = sectionName;
    if (options.clearFilter) {
      const search = document.getElementById('fieldManualSearch');
      if (search && search.value) {
        search.value = '';
        setFieldManualFilter('');
      }
    }
    applyFieldManualCompactState(options);
  }
  ...
  function showFieldManual(){
    ...
    try { localStorage.setItem('tapcalcAlpha214FieldManualOpen', '1'); } catch {}
  ...
  function installFieldManualReference(){
    ...
    document.body.classList.add('tapcalc-alpha214-fieldmanual');
    ...
    document.addEventListener('click', (event) => {
      ...
      if (jump) {
        ...
        setActiveFieldManualSection(target, { scroll: true, clearFilter: true, cssEscape });
        return;
      }
    ...
    document.addEventListener('change', (event) => {
      if (event.target?.id === 'fieldManualSectionSelect') {
        setActiveFieldManualSection(event.target.value, { scroll: true, clearFilter: true, cssEscape });
      }
    }, true);

    window.addEventListener('resize', () => applyFieldManualCompactState(), { passive: true });

    setTimeout(() => {
      ensureFieldManualTriggers();
      try {
        if (localStorage.getItem('tapcalcAlpha214FieldManualOpen') === '1') showFieldManual();
      } catch {}
    }, 450);
  }
