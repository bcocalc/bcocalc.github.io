/* TapCalc Dev 3.0.0-alpha211 inline field manual reference */
(function(){
  const STYLE_ID = 'tapcalc-alpha211-fieldmanual-style';
  const READY_FLAG = '__tapcalcAlpha211FieldManualReady';
  ...
  function showFieldManual(){
    ...
    try { localStorage.setItem('tapcalcAlpha211FieldManualOpen', '1'); } catch {}
  ...
  function hideFieldManual(){
    ...
    try { localStorage.removeItem('tapcalcAlpha211FieldManualOpen'); } catch {}
  }
  ...
  function installFieldManualReference(){
    ...
    document.body.classList.add('tapcalc-alpha211-fieldmanual');
    ...
    setTimeout(() => {
      ensureFieldManualTriggers();
      try {
        if (localStorage.getItem('tapcalcAlpha211FieldManualOpen') === '1') showFieldManual();
      } catch {}
    }, 450);
  }
  ...
  [250, 800, 1600, 2600].forEach((delay) => setTimeout(run, delay));
})();
